...
> 
> 💡 **灵感来源**：受 [OpenClaw](https://github.com/openclaw/openclaw) 的启发，MyClaw 专注于解决 AI 智能体的透明度和可控性问题。
...
MyClaw 支持**OpenClaw 技能**和**Claude Code 技能**，可直接使用两个生态系统的丰富技能资源，无需重新开发。
...
- **推荐技能**：
  - `skill-creator`：用自然语言让 MyClaw 自己创建技能
...
```bash
# 克隆项目
git clone https://github.com/ttguy0707/MyClaw.git
cd MyClaw
...
> 💡 **工作区配置**：工作区路径已在代码中初始化，默认为项目根目录的 `workspace` 文件夹，无需在 `.env` 中配置。仅当需要自定义工作区位置时，才设置 `MYCLAW_WORKSPACE` 环境变量。

> 💡 提示：配置完成后，可运行 `myclaw run` 聊天测试连接是否正常。
...
| 模块 | 文件 | 功能 |
|------|------|------|
| **Agent 循环** | `myclaw/core/agent.py` | LangGraph StateGraph，决策大脑 |
| **技能加载** | `myclaw/core/skill_loader.py` | 动态加载 SKILL.md，两段式调用 |
| **上下文管理** | `myclaw/core/context.py` | 消息修剪，双水位记忆 |
| **内置工具** | `myclaw/core/tools/builtins.py` | 时间/计算/任务调度等 |
| **沙盒工具** | `myclaw/core/tools/sandbox_tools.py` | 文件操作 + Shell 执行 |
| **审计日志** | `myclaw/core/logger.py` | JSONL 格式事件记录 |
| **心跳任务** | `myclaw/core/heartbeat.py` | 定时任务检查与触发 |
...
│   └── tasks.json                # 定时任务队列
├── logs/
│   └── local_myclaw_master.jsonl   # 审计日志
├── docs/                         # 文档与架构图
│   ├── architect.png             # 系统架构图
│   ├── monitor.png               # 监控终端截图
│   ├── welcome.png               # 欢迎界面
...
```bash
# 实时监控
tail -f logs/local_myclaw_master.jsonl

# 搜索特定事件
grep "tool_call" logs/local_myclaw_master.jsonl | tail -20
```
