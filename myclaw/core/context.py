...
def trim_context_messages(messages: list[BaseMessage], trigger_turns: int = 8, keep_turns: int = 4) -> tuple[list[BaseMessage], list[BaseMessage]]:
    # 按照完整用户回合来裁剪上下文：即 一个回合从HumanMessage开始，直到下一个HumanMessage结束，会把AIMessage、tool_calls、ToolMessage一并保留
    ...
    turns: list[list[BaseMessage]] = [] # 总回合
    current_turn: list[BaseMessage] = [] # 当前回合

    # 遍历非系统信息，按回合进行分组
    for msg in non_system_msgs:
        if isinstance(msg, HumanMessage): # 用户 -> 新回合
            if current_turn:
    ...
    total_turns = len(turns) # 按用户回合数裁剪
